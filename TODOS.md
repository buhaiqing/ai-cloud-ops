...
> 完整项目级 TODO tracker。M1 完成情况见底部 `## Completed (M1)`。
...
设计参考：[design.md § Milestone 2](./design.md) + [plan-eng-review TODOs](./design.md) T15 / T17

| # | 优先级 | 项目 | 描述 | 依赖 |
|---|---|---|---|---|
| M2-1 | **P1** | Next.js scaffolding | `web/` 目录 + Next.js 14 (app router) + TypeScript + Tailwind | M1 完成 |
...
| M2-7 | **P1** | 后端 REST API | FastAPI endpoints：GET /accounts, /resources, /alerts, /analyses；POST/PUT /rules | M1 backend |
...
设计参考：[design.md § Milestone 3](./design.md) + [plan-eng-review TODOs T16]()

| # | 优先级 | 项目 | 描述 | 依赖 |
|---|---|---|---|---|
| M3-1 | **P1** | T16 — ActionTrail ingestion | **Codex critical gap**：拉近期变更（10 min window）作为 AI 根因上下文；schema/permissions 已纳 db/migrations/0001_init.sql | T9 eval suite 跑通 |
| M3-2 | **P1** | 结构化操作清单 | recommendations 从"通用文本"升级为可机读 JSON：每个操作带 command + preconditions + rollback | agent client 升级 |
...
| M3-8 | **P2** | Eval suite 完整化 | 10 → 30+ 样本；CI gate 严格执行（avg ≥ 18/25） | T9 + T13 baseline |
...
| M4-2 | **P2** | T18 — Kill criterion + data retention | **Codex critical gap**：AI 准确率 < 16% kill；运维数据发给 LLM 的合规承诺（不存原始 payload / 30 天后删除） | M2 之前明确 |
...
| M4-12 | **P3** | Auto-remediation（受限操作 + 完整 audit） | M3-5 的全自动版本 | M4-5 + 强信任 |
...
| # | 来源 | 优先级 | 描述 |
|---|---|---|---|
| REVIEW-1 | Codex | **P0** | P1/P2 是断言不是验证——M2 启动前需要用户验证（5 个真实用户试用） |
| REVIEW-2 | Claude + Codex | **P1** | "AI 诊断" 必须可衡量（精度/可执行率/零幻觉率/响应时间）——T9 + docs/ai-quality.md 已立 |
| REVIEW-3 | Claude + Codex | **P2** | MCP 验证关卡：M1 完成后用 1 周做 MCP 最小原型 A/B test，决定 M2/M3 是否切 MCP |
| REVIEW-4 | Codex | **P1** | 2 周 MVP 不现实——M1 已重定义为 4 周 |
| REVIEW-5 | Codex | **P1** | 没有 kill criterion——T18 (M4-2) 在 M2 前必须定 |
...
| 🚧 | Worker loop (subagent running) | `/plan-eng-review` T7 | TBD |
| 🚧 | Resource fetcher (subagent running) | `/plan-eng-review` T3/T10 | TBD |
| 🚧 | AI Agent client (subagent running) | `/plan-eng-review` T2 | TBD |
| 🚧 | JSON logging + Quickstart (subagent running) | `/plan-eng-review` T7 | TBD |

---

**最后更新**：M1 实现期间（T+0 天，4 周 MVP 进度 30%）